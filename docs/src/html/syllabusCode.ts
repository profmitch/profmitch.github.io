...
interface SemesterInfo {
   ...
   firstSemesterDay: string; // "8/10/2026"
   censusDate: string; // "8/31/2026"
   sections: {
      section: number; // 25810
      lectureDays: "TTh" | "MW" | "MWF";
      lectureTimes: string;  // "5:00 pm - 6:20 pm"
      lectureLocation: string; // "NS 101"
      labDay: "M" | "T" | "W" | "Th"
      labTime: string; // "6:30 pm - 9:35 pm"
      labLocation: "string" // NS 339"
   }[];
}

async function getCourseInfo(): Promise<SemesterInfo> {
   ...
   // Parse and cast to your TypeScript interface
    const courseInfo: SemesterInfo = jsyaml.load(yamlText) as SemesterInfo;
    //console.log("first semester day is " + courseInfo.firstSemesterDay);
    return courseInfo;
}

function getCourseHeader(courseInfo: SemesterInfo): string {
   let out = "";
...
const getFinalExamDate = (firstSemesterDay: Date): string => {
...
function elementFilling(courseInfo: SemesterInfo, courseHeader: string) {
   const elementFills: {
      element: { node: "span" | "div", idtype: "id" | "class", idval: string; };
      attribs?: {name: string; val: string}[];
      content: string;
      }[] = [
            {
